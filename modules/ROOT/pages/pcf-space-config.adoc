...
Contact your MuleSoft Customer Success Manager for more information.
...
In the `space_settings.yml` file, configure the `pcf-services` and `pcf-cloud-params` properties as required.
...
The Space Configuration Tool uses the properties defined in `pcf-cloud-params` to register a space in an Anypoint Platform organization.
...
|COM_MULESOFT_ARMSERVICES_CLIENT_INSECURE | skips SSL validation. Set this to false only if your Anypoint Platform installation does not have valid HTTPS certificates.
...
----
+
The “uuid” is a unique value for a combination of space and Anypoint Platform organization. You must never delete `cloud-manager` as it is used by the Anypoint Platform to deploy apps to a PCF space.
