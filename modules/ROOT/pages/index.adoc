...
[Updated] API policies are supported, but requires the following additional setup:

Steps to do in PCE versions 1.6.x and 1.7.x:

Deploy customer app
Deploy Proxy App
Create User Service Instance for proxy app
Create a route between proxyApp and customer App
Start Using customer app url that is getting routed through proxy with policies applied

Deploy Customer App:
                Deploy app using “cf push <app>” with manifest file or usual cf commands
                After app is created, do cf app <app> and note down the url of the customer app.
      2. Deploy Proxy App:
               Require PCE instance to get properties.
...
After gathering all the properties, start deploying the proxy through following steps.
...
. cf set-env <proxy-app> MULE_JAVA_OPTS "-Dapi.name=muleapp -Dapi.version=1.0 -Dimplementation.host=muleapp.apps.santafesprings.cf-app.com -Dimplementation.port=80 -Dimplementation.path=/ -Danypoint.platform.client_id=0f2ea15d88fa452e8ca1a5f4bb724728 -Danypoint.platform.client_secret=BbCA70Bb056F437e94C707B67aF4284D -Danypoint.platform.platform_base_uri=https://161-171-upgrade-2056596225.us-east-2.elb.amazonaws.com/apiplatform -Danypoint.platform.coreservice_base_uri=https://161-171-upgrade-2056596225.us-east-2.elb.amazonaws.com/accounts -Danypoint.platform.on_prem=true -Danypoint.platform.contracts_base_uri=https://161-171-upgrade-2056596225.us-east-2.elb.amazonaws.com/apigateway/ccs"

. cf start <proxy>

Tail the logs cf logs <proxy>
You should see in logs    2018-11-13T10:07:09.52-0800 [APP/PROC/WEB/0] OUT INFO  2018-11-13 18:07:09,494 [agw-policy-polling.01] com.mulesoft.module.policies.lifecyle.GateKeeper: API <proxyappname> <version> is now unblocked.

And going to PCE instance API manager, proxy should auto registers and appear in with status as ACTIVE. If you see any 401 or 500 errors in the logs, make sure you have sufficient permissions to create an API in API manager.

At this step, we deployed proxy in PCF and auto registers /discovers at PCE instance using the properties that we provided in earlier step.

3. Create User instance for proxy app
    Ref: https://docs.cloudfoundry.org/services/route-services.html#user-provided
   cf create-user-provided-service SERVICE-INSTANCE -r ROUTE-SERVICE-URL 
Ex: cf create-user-provided-service muleappproxy -r https://muleappproxy.apps.santafesprings.cf-app.com

4. Bind or create route

cf bind-route-service DOMAIN SERVICE-INSTANCE [--hostname HOSTNAME]

Ex: cf bind-route-service apps.santafesprings.cf-app.com muleappproxy --hostname springbootapp

5. Validation
Try hitting the URL of the customerapp and see if it is still reachable and in proxy logs you should see lot of logs when you hit customerapp. And if it is not, then there is an issue with routing or proxy app

Configure policies for proxyAPI in PCE instance API manager. Try applying simple auth and httpauth and you should see policies getting applied logs in proxyapp logs.

Now try hitting the customerapp and it should now ask auth that you configured in API manager.
You can try other policies like throttling.


Extra steps in CloudHub (anypoint.mulesoft.com) and latest PCE instance 2.0.0:

In API manager 2.0, there are extra steps which are listed at ex: https://docs.mulesoft.com/api-manager/2.x/configure-autodiscovery-3-task
You should first register the API in API manager and API manager should provide API name and API version and these two properties should be used when configuring the properties in Step2.
     ...
     3. Also clientId and secret are for the environment instead of organization. Make sure you created the API in corresponding environment from which clientId and secret are provided to proxy.

Example property: cf set-env muleappproxy MULE_JAVA_OPTS "-Dapi.name=groupId:cef274d5-780e-48e2-9081-e98d5cfc6113:assetId:mm -Dapi.version=v1:136567 -Dimplementation.host=muleapp.apps.santafesprings.cf-app.com -Dimplementation.port=80 -Dimplementation.path=/ -Danypoint.platform.client_id=cdbff39bfa4741fb9bb531ba6102a578 -Danypoint.platform.client_secret=56bB958Bc257464B9fF0A9F9E6Af9890 -Danypoint.platform.platform_base_uri=https://qax.anypoint.mulesoft.com/apiplatform -Danypoint.platform.coreservice_base_uri=https://qax.anypoint.mulesoft.com/accounts -Danypoint.platform.on_prem=false -Danypoint.platform.contracts_base_uri=https://qax.anypoint.mulesoft.com/apigateway/ccs"
