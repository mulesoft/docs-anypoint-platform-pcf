...
The Anypoint Platform for Pivotal Cloud Foundry (Anypoint Platform for PCF) component is a fully integrated solution that enables organizations to design, develop, deploy, manage, and operate APIs, connectivity apps and microservices within the infrastructure provided by Anypoint Platform for PCF.

The Anypoint Platform for PCF solution enables you to:

* Deploy Mule applications to Anypoint Platform for PCF using the Runtime Manager UI.
* Create and deploy a Mule proxy application to Anypoint Platform for PCF. A proxy application intercepts traffic to a non-Mule application, enabling you to apply policies and view usage metrics.

If you are using Anypoint Platform for PCF and you have multiple Anypoint Platform for PCF installations, each must be linked to its own separate installation of Anypoint Platform Private Cloud Edition.

If you have an Anypoint Platform for PCF installation with multiple spaces, you must run the *Space Configuration Tool* on each of these spaces. If your Anypoint Platform organization uses multiple environments, it is recommended that you have one Anypoint Platform for PCF space for each.

Anypoint Platform for PCF uses the following services:

* **Runtime Manager Service**: exposes Anypoint Platform for PCF as a deployment target within the Anypoint Runtime Manager and lets you deploy Mule applications to Anypoint Platform for PCF, alongside other deployment targets.

* **API Gateway Service**: enables you to create and deploy a Mule API proxy that intercepts traffic to a non-Mule application running on Anypoint Platform for PCF. This allows you to use the MuleSoft interface to apply policies over the API and the Anypoint Platform for PCF interface to view usage and policy metrics.

* **Anypoint Metering Service**: provides an API that enables you to view usage metrics for Mule applications deployed to Anypoint Platform for PCF. You can view usage metrics for production, pre-production, staging, and development environments.
...
*** In the Anypoint Platform Private Cloud Edition instance of API Manager, the proxy auto registers with status as ACTIVE. If you see `401` or `500` errors in the logs, make sure you have sufficient permissions to create an API in API Manager. The proxy uses the properties provided in the previous step.
